...
export function LDLCalculatorForm({
  ...
  useEffect(() => {
    ...
    if (touched.colesterolTotal && colesterolTotal) {
      ...
      if (isNaN(value) || value < 100 || value > 500) {
        newErrors.colesterolTotal = "Colesterol total deve estar entre 100 e 500 mg/dL";
      }
    }
    ...
    if (touched.triglicerideos && triglicerideos) {
      ...
      if (isNaN(value) || value < 50 || value > 1000) {
        newErrors.triglicerideos = "Triglicerídeos devem estar entre 50 e 1000 mg/dL";
      }
    }
  ...
  const handleCalculate = async () => {
    ...
    // Validate all inputs
    const allTouched = { colesterolTotal: true, hdl: true, triglicerideos: true };
    setTouched(allTouched);

    const newErrors: Record<string, string> = {};

    if (!colesterolTotal) newErrors.colesterolTotal = "Colesterol total é obrigatório";
    if (!hdl) newErrors.hdl = "HDL é obrigatório";
    if (!triglicerideos) newErrors.triglicerideos = "Triglicerídeos são obrigatórios";
  ...
  const getRecommendations = (classification: string, ldlValue: number): string[] => {
  ...
  const canCalculate = colesterolTotal && hdl && triglicerideos && Object.keys(errors).length === 0;
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("colesterolTotal", e.target.value)}
                className={errors.colesterolTotal ? "border-red-500 focus:border-red-500" : ""}
                step="1"
                min="100"
                max="500"
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={errors.hdl ? "border-red-500 focus:border-red-500" : ""}
                step="1"
                min="20"
                max="100"
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("triglicerideos", e.target.value)}
                className={errors.triglicerideos ? "border-red-500 focus:border-red-500" : ""}
                step="1"
                min="50"
                max="1000"
              />
          ...
          {triglicerideos && parseFloat(triglicerideos) >= 400 && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                <strong>Atenção:</strong> Para triglicerídeos ≥ 400 mg/dL, a fórmula de Friedewald não é válida. 
                Considere dosagem direta de LDL ou fórmula de Martin-Hopkins.
              </AlertDescription>
            </Alert>
          )}
              ...
              <CardContent className="space-y-6">
                ...
                <div className="text-center">
                  <div className="text-4xl font-bold text-slate-900 mb-2">
                    {result.ldl.toFixed(1)} <span className="text-lg font-normal text-slate-600">mg/dL</span>
                  </div>
                  <Badge className={`text-sm px-3 py-1 ${getClassificationColor(result.classificacao)}`}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                    <span className="font-medium text-slate-700">Método utilizado:</span>
                    <Badge variant="outline" className="font-medium">
                      {result.method === "friedewald" ? "Friedewald" : "Martin-Hopkins"}
                    </Badge>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-gradient-to-r from-red-50 to-orange-50 rounded-lg border border-red-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold text-red-900">Risco Cardiovascular</span>
                    </div>
                    <p className="text-sm text-red-800">
                      {result.classificacao === "Ótimo" || result.classificacao === "Desejável" 
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold text-blue-900">Meta Terapêutica</span>
                    </div>
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {getRecommendations(result.classificacao, result.ldl).map((recommendation, index) => (
