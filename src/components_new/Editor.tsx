...
const Editor = ({ documentId }: EditorProps) => {
  ...
  useEffect(() => {
    ...
    const documentName =
      documentId ?? process.env.NEXT_PUBLIC_DOCUMENT_NAME ?? "example-document";
  ...
  return (
    <div className="mx-auto w-full max-w-4xl p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div
          className={`rounded-full px-2 py-1 text-white ${status === "connected" ? "bg-green-500" : "bg-red-500"}`}
        >
          {status}
        </div>
      </div>
      <MenuBar editor={editorRef.current} />
      <div className="prose prose-lg max-w-none rounded-md border p-4">
