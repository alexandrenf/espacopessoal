...
interface SidebarProps {
  ...
  showSidebar?: boolean;
  onUpdateStructure?: (
    structure: { id: number; parentId: number | null; order: number }[],
  ) => void;
  isMobile?: boolean;
}
  ...
  ({
    ...
    const handleDrop = (info: TreeDropInfo) => {
      ...
      if (dropPosition === 0 && dropNote.isFolder) {
        ...
        const dragNoteIndex = updatedNotes.findIndex(
          (n) => n.id === dragNote.id,
        );
        ...
        updatedNotes.forEach((note) => {
          if (
            note.parentId === dragNote.parentId &&
            note.order > dragNote.order
          ) {
    ...
    return (
      <section
        className={`flex h-full w-full flex-col bg-white md:h-screen ${isMobile ? "fixed bottom-0 left-0 right-0 top-16 z-50" : ""}`}
      >
        <div className="flex shrink-0 items-center justify-between border-b border-gray-200 bg-white/80 p-4 backdrop-blur-md">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  className="border-blue-200 bg-blue-50 text-blue-700 hover:border-blue-300 hover:bg-blue-100 active:bg-blue-200"
                >
                  {isCreating ? (
